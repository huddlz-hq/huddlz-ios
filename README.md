...
The app opens on Discover, the search tab, which keeps the discovery journey below unchanged. While Discover is selected its search field is expanded in the tab bar and the other tabs fold into one pill; tapping the pill opens Agenda and shows Agenda and Groups with the Discover search button beside them. Tapping that button returns to Discover with the field open. Scrolling down shrinks the bar and minimizes the field; scrolling up brings them back. Switching tabs and back keeps the search text, filters, and results in place. Signed-out visitors see what each tab will hold and a “Sign in” action that opens the account sheet in place; the tab checks the saved session before offering sign-in. The selected tab survives rotation and backgrounding through scene storage; the app does not promise it across a relaunch.

## Agenda

The Agenda tab opens under the huddlz wordmark and a glass Account button, with “Your agenda” and a count of the huddlz coming up once they load. It lists the upcoming huddlz the signed-in member is going to or waitlisted for, soonest first, with adjacent huddlz grouped by day in each huddl’s own time zone; each day’s huddlz share one card under a small heading. A day heading can repeat when time zones cross date boundaries, keeping the list chronological. Each row shows a glass date stamp, the title, the start time with the event’s zone, the location, and a Going pill in the accent or a Waitlisted pill in the warm hybrid tone. Tap a row to open the same huddl page discovery uses; native back returns to the agenda. Members with no upcoming RSVPs read that RSVPing to a huddl in Discover adds it here. A failed request offers retry. RSVP changes made in the app reload the agenda, so a cancelled RSVP disappears on return; changes made on the website appear on the next load or retry. Signing out returns the tab to the sign-in prompt and clears the list.
...
Browse real huddlz in cards that share the web app’s anatomy: a 16:9 cover with a glass date stamp (month over day) and a glass tag, then the hosting group when known, the title, and a meta line with the weekday, start time, event zone, and location. The tag names the event type (in person, online, hybrid) in that type’s color; when signed in it shows “Going” or “Waitlisted” instead and the event type moves into the meta line. Cards without an RSVP keep the type tag. Filters are glass chips that take the accent color only while a filter is set. Badges refresh on app return, pull-to-refresh, and as cards enter view, and disappear after sign-out. Submit text with the keyboard’s Search action, and filter by event type (all, in person, online, hybrid) and date (all upcoming, this week, this month). Clear the search field to reset the text query. Pull to refresh while keeping the current cards visible. A failed refresh keeps the cards and filters and offers retry. Search uses the search tab’s native field. Native pull-to-refresh keeps working alongside it in the tab layout (verified September 12, 2026); the earlier UIKit search bar existed because a top-placed SwiftUI search field detached the refresh control. Scroll near the last card to load the next batch. A bottom spinner shows progress; a failed batch keeps the cards and offers retry. Loading stops after the final batch. Opening a card fetches current details separately. The huddl page opens on a full-width cover that fades into the page, then the event-type pill and title, the RSVP row with its one action, a card with the hosting group and the day and time range in the event’s zone (an end on a later day is spelled out, and the zone identifier sits beneath), the venue map or online label, the description, and online joining. Details show the hosting group in that card. Tap it to read the group’s description and location and browse its upcoming huddlz. Native back navigation returns through the group to the original huddl. Empty lists explain that no upcoming huddlz are scheduled; failed requests offer retry. Load more adds the next batch without removing existing cards. Missing or withheld host information leaves the rest of the details available without a host row. Signed-in details show whether you are going, waitlisted, or have not RSVP’d. The status refreshes when details open or the app becomes active, including changes made on the website. Anonymous browsing shows no personal-status claim. Failed or unreadable status requests keep the event visible with retry.
...
Design reference: `prototype/issue-1-core-journey`, variant A (cards). The prototype and simulated account/RSVP flow remain on that branch. The palette follows the huddlz web app (`assets/css/app.css` in the `huddlz` repo): teal accent, pink for online huddlz, and a warm tone for hybrid ones, each with light and dark values in the asset catalog. Artwork tests recognize the fallback illustration by its teal accent pixels and measure the cover below the date stamp and tag.
